...
def data_validation(data_dict: dict):
    ...
    if data_dict["WIDTH"] == 0:
        raise ErrorInConfigFile(
            f"Invalid WIDTH: {data_dict['WIDTH']}." " WIDTH must be greater than 0."
        )
    if data_dict["HEIGHT"] == 0:
        raise ErrorInConfigFile(
            f"Invalid HEIGHT: {data_dict['HEIGHT']}." " HEIGHT must be greater than 0."
        )

    # ENTRY and EXIT validation
    if ("," in data_dict["ENTRY"] and "," in data_dict["EXIT"]) is False:
        raise ErrorInConfigFile("ENTRY and EXIT  should be with this format " "X,Y")
    data_dict["ENTRY"] = data_dict["ENTRY"].split(",")
    data_dict["EXIT"] = data_dict["EXIT"].split(",")

    if (data_dict["ENTRY"][0].isdigit() and data_dict["ENTRY"][1].isdigit()) is False:
        raise ErrorInConfigFile(
            f"Invalid ENTRY coordinates: {data_dict['ENTRY']}."
            " Coordinates must be numbers."
        )
    if (data_dict["EXIT"][0].isdigit() and data_dict["EXIT"][1].isdigit()) is False:
        raise ErrorInConfigFile(
    ...
    if data_dict["ENTRY"].__len__() != 2:
        raise ErrorInConfigFile("ENTRY coordinates should be at this format" "X,Y")
    if data_dict["EXIT"].__len__() != 2:
        raise ErrorInConfigFile("EXIT coordinates should be at this format" "X,Y")
    if (
...
def validation(text: str):
    ...
    for line in lines_w:
        if line.__len__() != 2:
            raise ErrorInConfigFile(
                f"Invalid line: '{line}.'" " Each line must be a 'KEY=VALUE' pair."
            )
    ...
    if keys != valid_keys.__len__():
        raise ErrorInConfigFile(
            "A mandatory key is missing, " f"verify your file again. {valid_keys}"
        )
